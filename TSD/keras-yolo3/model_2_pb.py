...
def _main_(args):
    ...
    try:
        model_outputs = [tensor.name.split(':')[0] for tensor in infer_model.output]
    except TypeError:
        model_outputs = [infer_model.output.name.split(':')[0]]

    model_input_classes = infer_model.input.shape[3]
    ...
    with K.get_session() as sess:
        ...
        # image = tf.placeholder("float", [1, 416, 416, 3], name="input_img_new")

        # _ = tf.import_graph_def(graphdef, input_map={"input_img": image})
