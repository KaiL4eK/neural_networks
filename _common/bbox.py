...
class BoundBox:
    ...
    def __str__(self):
        return "[{}:{}, {}:{} / classes: {} / label: {} ({})]".format(self.ymin, self.ymax, self.xmin, self.xmax, self.classes, self.class_name, self.class_idx)

    def __repr__(self):
        return "[{}:{}, {}:{} / classes: {} / label: {} ({})]".format(self.ymin, self.ymax, self.xmin, self.xmax, self.classes, self.class_name, self.class_idx)

    def as_voc_list(self):
    ...
    for box in boxes:
        ...
        for i in range(len(labels)):
            if box.classes[i] > obj_thresh:
                if label_str != '':
                    label_str += ', '
                label_str += (labels[i] + ' ' +
                              str(round(box.get_best_class_score()*100, 2)) + '%')
                label = i
            if not quiet:
                print(label_str)
        ...
        if label >= 0:
            ...
            text_size = cv2.getTextSize(
                label_str, cv2.FONT_HERSHEY_SIMPLEX, font_scl, thickness)
            ...
            cv2.rectangle(img=image, pt1=(box.xmin, box.ymin), pt2=(
                box.xmax, box.ymax), color=get_color(label), thickness=3)
...
def draw_ros_boxes(image, boxes, labels, obj_thresh, quiet=True):
    for box in boxes:
        ...
        if label >= 0:
            ...
            cv2.rectangle(img=image, pt1=(box.xmin, box.ymin), pt2=(
                box.xmax, box.ymax), color=get_color(label), thickness=5)
            cv2.fillPoly(img=image, pts=[region], color=get_color(label))
            cv2.putText(img=image,
                        text=label_str,
                        org=(box.xmin+13, box.ymin - 13),
