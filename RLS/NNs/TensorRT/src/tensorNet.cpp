...
MyEngine* createTrtFromUFF(char* uff_path, char* input, char* output)
{
    ...
    parser->registerInput(input, DimsCHW(3, 160, 320), UffInputOrder::kNHWC);
    ...
    if (!parser->parse(uff_path, *network, nvinfer1::DataType::kFLOAT)) 
    {
        std::cout << "Fail to parse UFF model " << uff_path << std::endl;
        exit(0);
    }
    ...
    std::cout << "[cpp] Successfully create TensorRT engine from file " << uff_path << std::endl;
...
void getOutput(MyEngine* my_engine, int out_idx, int rows, int cols, int chnls, float* data_out)
{
    ...
    std::cout << output_dims.d[0] << std::endl;
    std::cout << output_dims.d[1] << std::endl;
    std::cout << output_dims.d[2] << std::endl;
    std::cout << rows << std::endl;
    std::cout << cols << std::endl;
    std::cout << chnls << std::endl;

    assert( output_dims.nbDims == 3 );
    assert( output_dims.d[0] == rows );
    assert( output_dims.d[1] == cols );
    assert( output_dims.d[2] == 1 );
